...
def calcular_valores_combinaciones(matrix, value, h):
    ...
    n = calcular_grado_maximo_polinomio(matrix)
    s = float((value - valores_x[-1]) / h)
    for i in range(1, n + 1):
        j = s
        m = 1
        L = (s + (i - 1)) #calcular el límite y meterlo en el otro ciclo
        while j <= L:
            m *= j
            j += 1
        valores_combinaciones.append((m)/(math.factorial(i)))
    print(valores_combinaciones)
...
def ejecutar_metodo():
    ...
    df = pd.read_csv('C:\\Users\\EDGAR\\Desktop\\EDGAR\\SCHOOL\\TEC DE MORELIA\\TERCER SEMESTRE\\MÉTODOS NUMÉRICOS\\PYTHON\\UNIDAD 4\\interpolacion\\datos\\Datos_Newton_Hacia_Atras.csv', delimiter=',')
    matrix = df.to_numpy()
...
def main():
    print("Bienvenid@ al método de Newton - Hacia Atrás")
    print("El archivo que contiene los datos tiene el nombre de 'Datos_Newton_Hacia_Atrás.csv'")
