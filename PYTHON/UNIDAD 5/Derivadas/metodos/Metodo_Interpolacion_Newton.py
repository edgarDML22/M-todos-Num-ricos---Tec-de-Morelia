...
def buscar_equidistancia(matrix, x_value):
    index_list = []
    ...
    if 0 in index_list:
        if (valores_x[1] - valores_x[0]) == (valores_x[2] - valores_x[1]):
            index_list.append(2)
    #si da el último
    elif n - 1 in index_list:
        if (valores_x[n - 1] - valores_x[n - 2]) == (valores_x[n - 2] - valores_x[n - 3]):
            index_list.append(n - 2)
    else:
        #checar el anterior y el siguiente
        posibles_index = [index_list[0] - 1, index_list[1] + 1]
        #checar el anterior
        if (valores_x[index_list[1]] - valores_x[index_list[0]]) == (valores_x[index_list[0]] - valores_x[posibles_index[0]]):
            index_list.append(posibles_index[0])
        if (valores_x[index_list[1]] - valores_x[index_list[0]]) == (valores_x[posibles_index[1]] - valores_x[index_list[1]]) and len(index_list) == 2:
            index_list.append(posibles_index[1])
    #ordenar la lista
    index_list = sort_list(index_list.copy())
    index_list = [int(index) for index in index_list]
    return index_list

def calcular_derivadas_interpolacion_Newton(matrix, index_list, x_value):
    ...
    derivatives = []
    h = nuevos_valores_x[1] - nuevos_valores_x[0]
    k = (x_value - nuevos_valores_x[0])/h
    #calcular la primera derivada
    derivatives.append((1/h)*((nuevos_valores_y[1] - nuevos_valores_y[0]) + ((2*k - 1)/2)*(nuevos_valores_y[2] - 2*nuevos_valores_y[1] + nuevos_valores_y[0])))
    #calcular la segunda derivada
    derivatives.append((1/(math.pow(h, 2)))*(nuevos_valores_y[2] - 2*nuevos_valores_y[1] + nuevos_valores_y[0]))
...
def ejecutar_metodo():
    ...
    df = pd.read_csv('C:\\Users\\EDGAR\\Desktop\\EDGAR\\SCHOOL\\TEC DE MORELIA\\TERCER SEMESTRE\\MÉTODOS NUMÉRICOS\\PYTHON\\UNIDAD 5\\Derivadas\\datos\\Datos_Interpolacion_Newton.csv', delimiter=',')
    matrix = df.to_numpy()
    valores_x = matrix[:,matrix.shape[1] - 2].tolist()
    #Pedir el valor de x
    ...
    while True:
        ...
        if valores_x[0] <= x_value <= valores_x[-1]:
            ...
            index_list = buscar_equidistancia(matrix, x_value)
            if(len(index_list) != 2):
                print("Valor adecuado para el método")
                #hacer las operaciones
                derivatives = calcular_derivadas_interpolacion_Newton(matrix, index_list, x_value)
...
def main():
    ...
    print("El archivo que contiene los datos tiene el nombre de 'Datos_Interpolacion_Newton.csv'")
