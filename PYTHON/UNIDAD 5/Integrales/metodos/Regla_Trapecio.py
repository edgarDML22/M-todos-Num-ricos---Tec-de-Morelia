...
def ejecutar_metodo_con_funcion(function_str, n, a, b):
    valores_y = calcular_valores_y_Trapecio(function_str, n, a, b)
    suma_valores_y = calcular_suma_valores_y_Trapecio(valores_y)
    h = (b - a)/n
    resultado = (h/2)*(suma_valores_y)
    print("El valor de la integral es:")
    print(f"I = {valor_cifras_significativas(resultado, 6) }")

def ejecutar_metodo_tabla_valores():
    df = pd.read_csv('C:\\Users\\EDGAR\\Desktop\\EDGAR\\SCHOOL\\TEC DE MORELIA\\TERCER SEMESTRE\\MÉTODOS NUMÉRICOS\\PYTHON\\UNIDAD 5\\Integrales\\datos\\Datos_Regla_Trapecio.csv', delimiter=',')
    matrix = df.to_numpy()
...
def main():
    print("Bienvenid@ al método del Trapecio para calcular integrales")
    while True:
    ...
    if ans == 1:
        ...
        ejecutar_metodo_con_funcion(function_str, n, a, b)
    else:
        print("El archivo que contiene los datos tiene el nombre de 'Datos_Regla_Trapecio.csv'")
