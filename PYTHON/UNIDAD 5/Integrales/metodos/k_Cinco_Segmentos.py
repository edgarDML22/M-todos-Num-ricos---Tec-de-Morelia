...
def calcular_integral_5k_segmentos(valores_x, valores_y):
    valor_integral = 0
    n = int((len(valores_y) - 1) / 5)
    #5*i y 5*(i+1) + 1 = 5*i + 6
    for i in range(n):#numero de paquetes
        suma_parcial_valores_y = 0
        a = valores_x[5*i]
        b = valores_x[5*(i+1)]
        for j in range(5*i, 5*i + 6):
            if j == 5*i or j == 5*(i+1):
                suma_parcial_valores_y += 19*valores_y[j]
            elif j == 5*i + 1 or j == 5*(i+1) - 1:
                suma_parcial_valores_y += 75*valores_y[j]
            else:
                suma_parcial_valores_y += 50*valores_y[j]
        valor_integral += ((b-a)/288)*(suma_parcial_valores_y)
...
def ejecutar_metodo_5k_segmentos_tabla():
    df = pd.read_csv('C:\\Users\\EDGAR\\Desktop\\EDGAR\\SCHOOL\\TEC DE MORELIA\\TERCER SEMESTRE\\MÉTODOS NUMÉRICOS\\PYTHON\\UNIDAD 5\\Integrales\\datos\\Datos_k_Cinco_Segmentos.csv', delimiter=',')
    matrix = df.to_numpy()
    valores_y = matrix[:,matrix.shape[1] - 1].tolist()
    valores_x = matrix[:,matrix.shape[1] - 2].tolist()
    if (len(valores_x) - 1) % 5 == 0:
        ...
        print("El número de segmentos de la tabla no es un múltiplo de 5")
        print("Este método sólo funciona con segmentos múltiplos de 5")
        


def main():
    print("Bienvenid@ al método para k*5 segmentos para calcular integrales")
    while True:
    ...
    if ans == 1:
        ...
        while True:
            ...
            print("Para este método el número de segmentos debe ser MULITPLO DE 5")
            n = ask_for_int("valor de n")
            if n >= 1:
                if n % 5 == 0:
                    break
                else:
                    print("El número de segmentos ingresados no es múltiplo de 5") 
        ...
        ejecutar_metodo_5k_segmentos(function_str, n, a, b)

    else:
        print("El archivo que contiene los datos tiene el nombre de 'Datos_k_Cinco_Segmentos.csv'")
        ...
        if ans2 == 1:
            ...
            ejecutar_metodo_5k_segmentos_tabla()
        else:
